...
BASE_URI = "http://www.semanticweb.org/kemp/resource/"

def uri(cls, id_val):
    """Create a full resource URI."""
    safe_id = str(id_val).replace(' ', '_').replace('\\', '_').replace('"', '_')
    return f"<{BASE_URI}{cls}/{safe_id}>"
...
def int_literal(val):
    """Format an integer with xsd:int type (matching ontology)."""
    return f'"{int(val)}"^^xsd:int'

def float_literal(val):
    """Format a float with xsd:float type (matching ontology)."""
    return f'"{float(val)}"^^xsd:float'

def decimal_literal(val):
    """Format a decimal value with xsd:decimal type."""
    return f'"{float(val)}"^^xsd:decimal'

def bool_literal(val):
    """Format a boolean with xsd:boolean type."""
    return f'"{val}"^^xsd:boolean'

def parse_academic_year(ay_str):
    """Convert '2013-2014' to start year int."""
...
def safe_bool(val):
    """Convert to boolean if possible, else None."""
    if not val or not val.strip():
        return None
    v = val.strip().lower()
    if v in ('true', '1', 'yes'):
...
def process_students(filepath):
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            triples[subj].append(f':personalID {int_literal(sid.replace("-", ""))} .')
            ...
            yr = safe_int(year)
            if yr is not None:
                triples[enr_subj].append(f':enrolledYear {int_literal(yr)} ;')
            
            grad = safe_bool(graduated)
            if grad is not None:
                triples[enr_subj].append(f':graduated {bool_literal(grad)} ;')
...
def process_teaching_assistants(filepath):
    """Teaching_Assistants.csv -> :Teacher"""
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            for row in reader:
                ...
                triples[subj].append(f':name "{esc(name)}" ;')
                triples[subj].append(f':personalID {int_literal(tid.replace("-", ""))} ;')
...
def process_programme_courses(filepath):
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            sy = safe_int(syear)
            if sy is not None:
                triples[subj].append(f':studyYear {int_literal(sy)} ;')
            
            if year_int:
                triples[subj].append(f':academicYear {int_literal(year_int)} ;')
...
def process_courses(filepath):
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if cr is not None:
                triples[subj].append(f':credits {float_literal(cr)} ;')
...
def process_course_instances(filepath):
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            sp = safe_int(speriod)
            if sp is not None:
                triples[subj].append(f':studyPeriod {int_literal(sp)} ;')
...
def process_course_plannings(filepath):
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            pl = safe_int(planned)
            if pl is not None:
                triples[subj].append(f':planningNumStudents {int_literal(pl)} ;')
            
            sh = safe_float(senior)
            if sh is not None:
                triples[subj].append(f':seniorHours {float_literal(sh)} ;')
            
            ah = safe_float(assistant)
            if ah is not None:
                triples[subj].append(f':assistantHours {float_literal(ah)} .')
...
def process_registrations(filepath):
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            gr = safe_float(grade)
            if gr is not None:
                triples[subj].append(f':grade {float_literal(gr)} ;')
...
def write_triples(output_dir='output'):
    ...
    # Combine all triples per subject into proper blocks
    ...
    if teaching_hours_map:
        with open(os.path.join(output_dir, 'teaching_hours.ttl'), 'w', encoding='utf-8') as f:
            ...
            for (tid, iid), (assigned, reported) in teaching_hours_map.items():
                ...
                f.write(f'    :assignedHours {float_literal(assigned)} ;\n')
                f.write(f'    :reportedHours {float_literal(reported)} ;\n')
