...
def int_literal(val):
    """Format an integer with explicit xsd:integer type."""
    return f'"{val}"^^xsd:integer'

def float_literal(val):
    """Format a float with explicit xsd:decimal type."""
    # Check if it's a whole number
    if val == int(val):
        return f'"{int(val)}"^^xsd:decimal'
    else:
        return f'"{val}"^^xsd:decimal'

def bool_literal(val):
    """Format a boolean with explicit xsd:boolean type."""
